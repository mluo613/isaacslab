...
public class ScatterPlotAcceleration : MonoBehaviour {
    ...
    void plotPoint(float pointPosition, UnityEngine.Color color)
    {
        ...
        GameObject tablet = GameObject.Find("AxesAcceleration");
    ...
    void Update () {
        if (timerStarted && mechanicsScript.enableMotion)
        {
            ...
            if(timeElapsed >= timeInterval * numberPointsCollected)
            {
                ...
                {
                    plotPoint(0, new UnityEngine.Color(250, 0, 0));
                }
                else
                {
